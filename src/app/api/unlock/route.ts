...
const input = z.object({
  sectionId: z.string().regex(/^[a-z0-9-]{1,60}$/),
...
export const POST = endpoint(async (request) => {
  ...
  const { sectionId, answer } = input.parse(await body(request));
  const result = await unlock(session.sub, sectionId, answer);
  await issueSession(session.sub, result.unlocked);
  return json({ text_en: result.text_en, revisit: result.revisit });
});
