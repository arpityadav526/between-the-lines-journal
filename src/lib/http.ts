    ...
    try {
      if (request.method !== "GET") {
        ...
        if (!origin || origin !== new URL(request.url).origin)
          throw new HttpError(403, "Please open this form from the journal.");
        if (
...
export async function body(request: Request) {
  ...
  if (!reader) throw new HttpError(400, "Missing input.");
  let value = "";
  let bytes = 0;
  ...
  while (true) {
    ...
    if (bytes > 4096) {
      await reader.cancel();
      throw new HttpError(413, "Input is too long.");
    }
