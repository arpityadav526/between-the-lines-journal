...
export function encrypt(text: string, context: string) {
  ...
  const cipher = createCipheriv(
    "aes-256-gcm",
    Buffer.from(env().CONTENT_KEY, "hex"),
    iv,
  );
...
export async function hashAnswer(
  ...
  return {
    salt,
    hash: ((await derive(answer, salt, 64)) as Buffer).toString("hex"),
  };
}
export async function verifyHash(
  ...
  return safeEqual((await hashAnswer(answer, entry.salt)).hash, entry.hash);
}
